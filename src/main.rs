    ...
    pub fn update(&mut self, input: &[u8]) {
        ...
        for chunk in unprocessed.chunks(16) {
            ...
            for (j, &c) in chunk.iter().enumerate() {
                self.checksum[j] ^= MD2_S_TABLE[(c ^ L) as usize];
                L = self.checksum[j];
            }
        }
    }
...
fn main() {
    // Try splitting up into individual cycles of update
    //let mut md2 = MD2::new();
    //let test_seq = &b"abcdefghijklmnopqrstuvwxyz"[..16];
    //dbg!(&String::from_utf8(test_seq.to_owned()).unwrap());
    //dbg!(test_seq.len());
    //for (outer_cycle, chunk) in test_seq.chunks(1).enumerate() {
        //eprintln!("outer_cycle = {}, intermediate checksum = {:?}", outer_cycle, md2);
        //md2.update(chunk);
    //}
    //dbg!(md2);
    dbg!(MD2::with_input(b""));
    dbg!(MD2::with_input(b"a"));
    dbg!(MD2::with_input(b"abc"));
    dbg!(MD2::with_input(b"message digest"));
    dbg!(MD2::with_input(b"abcdefghijklmnopqrstuvwxyz"));
    dbg!(MD2::with_input(b"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"));
    dbg!(MD2::with_input(b"12345678901234567890123456789012345678901234567890123456789012345678901234567890"));
}
